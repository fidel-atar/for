...
export default function AdminPlayerCrudScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await dataService.updatePlayer(editingId, playerData);
  ...
  const duplicatePlayer = async (item) => {
    try {
      ...
      // Create new player
      await dataService.createPlayer(playerData);
      Alert.alert('Success', 'Player duplicated successfully');
          ...
          <CrudList
            ...
            renderItemFields={(item) => (
              <View style={styles.playerListItem}>
                {item.image_url ? (
                  <Image source={{ uri: item.image_url }} style={styles.playerListImage} />
                ) : (
