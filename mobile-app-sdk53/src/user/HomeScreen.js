...
export default function HomeScreen({ navigation }) {
  ...
  const loadFeaturedNews = async () => {
    try {
      setLoading(true);
      const response = await dataService.getFeaturedNews();
  ...
  const renderStatCard = (icon, number, label, onPress, index) => {
    const statCardFadeAnim = React.useRef(new Animated.Value(0)).current;
    const statCardScaleAnim = React.useRef(new Animated.Value(0.9)).current;
    ...
    useEffect(() => {
      Animated.parallel([
        ...
        Animated.timing(statCardScaleAnim, {
          toValue: 1,
          duration: 600,
          delay: 300 + index * 100,
          useNativeDriver: true,
    ...
    return (
      <Animated.View
        style={[
          ...
          { 
            opacity: statCardFadeAnim, 
            transform: [{ scale: statCardScaleAnim }]
          }
        ]}
      >
        <TouchableOpacity 
  ...
  const renderMatchCard = (match, index) => {
    // Staggered animation for each card
    const matchCardFadeAnim = React.useRef(new Animated.Value(0)).current;
    const matchCardSlideAnim = React.useRef(new Animated.Value(20)).current;
    
    useEffect(() => {
      Animated.parallel([
        Animated.timing(matchCardFadeAnim, {
          toValue: 1,
          duration: 600,
          delay: index * 100,
          useNativeDriver: true,
        }),
        Animated.timing(matchCardSlideAnim, {
          toValue: 0,
          duration: 600,
          delay: index * 100,
          useNativeDriver: true,
    ...
    return (
      <Animated.View
        ...
        style={[
          ...
          { 
            opacity: matchCardFadeAnim, 
            transform: [{ translateY: matchCardSlideAnim }]
          }
        ]}
      >
        <TouchableOpacity
        ...
        >
          ...
          <View style={styles.teamsContainer}>
            <View style={styles.teamSection}>
              <Text style={styles.teamName}>{match.homeTeam || 'فريق أ'}</Text>
            ...
            <View style={styles.teamSection}>
              ...
              <Text style={styles.teamName}>{match.awayTeam || 'فريق ب'}</Text>
            </View>
          </View>
