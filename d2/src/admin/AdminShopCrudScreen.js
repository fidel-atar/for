...
export default function AdminShopCrudScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let result;
      if (editingId) {
        result = await dataService.updateShopItem(editingId, productData);
      } else {
        result = await dataService.createShopItem(productData);
      }
  ...
  const duplicateProduct = async (item) => {
    try {
      ...
      // Create new product
      const result = await dataService.createShopItem(productData);
      if (result.success) {
        Alert.alert('Success', result.message || 'Product duplicated successfully');
        // Reload list
        listRef.current?.reload();
      } else {
        Alert.alert('Error', result.message || 'Failed to duplicate product');
      }
    } catch (err) {
