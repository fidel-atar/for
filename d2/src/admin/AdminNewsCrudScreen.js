...
export default function AdminNewsCrudScreen() {
  ...
  const duplicateNews = async (item) => {
    try {
      ...
      if (result.success) {
        Alert.alert('Success', result.message || 'News article duplicated successfully');
        // Reload list
        listRef.current?.reload();
      } else {
        Alert.alert('Error', result.message || 'Failed to duplicate news article');
      }
    } catch (err) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* News List Section - Outside ScrollView to avoid VirtualizedList nesting */}
      <View style={styles.listContainer}>
        <View style={styles.listHeader}>
          <Ionicons name="list-outline" size={24} color="#1e3c72" />
          <Text style={styles.listTitle}>Existing Articles</Text>
