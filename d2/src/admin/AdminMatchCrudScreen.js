...
export default function AdminMatchCrudScreen() {
  ...
  const duplicateMatch = async (item) => {
    try {
      ...
      // Create new match
      const result = await dataService.createMatch(matchData);
      if (result.success) {
        Alert.alert('Success', result.message || 'Match duplicated successfully');
        // Reload list
        listRef.current?.reload();
      } else {
        Alert.alert('Error', result.message || 'Failed to duplicate match');
      }
    } catch (err) {
